...
class TestOauth2ClientViewsTestCase(TestCase):
    ...
    @mock.patch(
        "locarise_drf_oauth2_support.oauth2_client.backends.LocariseOAuth2.get_json"
    )  # noqa
    def test_oauth_obtain_auth_token(self, get_json):

        # 1. bad requests
        url = reverse("rest_framework:oauth2-token", kwargs={"backend_name": "unknown"})
        payload = {"email": "john@example.com", "access_token": "john_access_token"}
        res = self.client.post(url, data=payload, format="json")
        ...
        url = reverse(
            "rest_framework:oauth2-token", kwargs={"backend_name": "locarise-oauth2"}
        )
        res = self.client.post(url, data=payload, format="json")
        self.assertEqual(res.status_code, 403)

        payload = {"access_token": "john_access_token"}
        res = self.client.post(url, data=payload, format="json")
        self.assertEqual(res.status_code, 400)
        ...
        get_json.return_value = {
            ...
            "locale": "en-us",
            "organizations": [
                {"uid": "orga-uid", "name": "orga-name", "role": "manager"}
            ],
        }
